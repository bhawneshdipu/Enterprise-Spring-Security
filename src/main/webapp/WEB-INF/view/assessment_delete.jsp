...
<body>
	...
	<div class='col-lg-8 col-md-offset-2'>
		<form class="form-horizontal" action="/admin/assessment/delete/${sessionScope.assessment.eid}"
			method="post">
				...
				<div class="col-sm-10">
					<select class="form-control" id="sid" name="sid">
						<c:forEach var="student" items="${studentList}">
							<option value="${student.sid}" ${student.sid == sessionScope.assessment.sid ? 'selected="selected"' : ''}>${student.fname} ${student.surname}</option>
						</c:forEach>
					</select>
				</div>
			</div>
				...
				<div class="col-sm-10">
					<select class="form-control" id="eid" name="eid">
						<c:forEach var="enrollment" items="${sessionScope.enrollmentList}">
							<option value="${enrollment.eid}" ${enrollment.eid == sessionScope.assessment.eid ? 'selected="selected"' : ''}>${enrollment.semester}</option>
						</c:forEach>
					</select>
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<input type="text" class="form-control" id="a1"
						name="a1" placeholder="Enter A1" value="${sessionScope.assessment.a1}" />
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<input type="text" class="form-control" id="a2"
						name="a2" placeholder="Enter A2" value="${sessionScope.assessment.a2}" />
				</div>
			</div>
